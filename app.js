...
/** -----------------------------------------------------
 *  HELPERS PARA BUSCAR Y ORDENAR FICHAS TÉCNICAS
 *  (Se usan en la búsqueda y listado de fichas)
 * ----------------------------------------------------- **/

function getCampoFicha(obj, ...opts) {
  if (!obj) return '';
...
function scoreFichaMatch(row, rawQuery) {
  const query = String(rawQuery || '').trim();
  if (!query) return 0;

  const qNameNorm = normalizeNameFull(query);
  const qDigits   = query.replace(/\D+/g, '');

  const nombre    = getCampoFicha(row, 'nombre','cliente');
  const telefono  = getCampoFicha(row, 'telefono','tel','phone');

  const nNorm  = normalizeNameFull(nombre);
  const tDigits = String(telefono || '').replace(/\D+/g, '');

  let nameScore = 0;
  if (nNorm && qNameNorm) {
    if (nNorm === qNameNorm)      nameScore = 3;
    else if (nNorm.includes(qNameNorm)) nameScore = 2;
    else if (qNameNorm.includes(nNorm)) nameScore = 1;
  }

  let telScore = 0;
  if (tDigits && qDigits) {
    if (tDigits === qDigits) telScore = 3;
    else if (tDigits.endsWith(qDigits) || qDigits.endsWith(tDigits)) telScore = 2;
    else if (tDigits.includes(qDigits) || qDigits.includes(tDigits)) telScore = 1;
  }

  return telScore * 10 + nameScore;
}

function sortFichasByQuery(rows, query) {
  if (!query || !query.trim()) return rows;

  const copia = Array.isArray(rows) ? [...rows] : [];
  copia.sort((a, b) => {
    const scoreB = scoreFichaMatch(b, query);
    const scoreA = scoreFichaMatch(a, query);
...
async function loadFichas() {
  try {
    ...
    // 1) Traer todas las fichas del tipo actual
    let rows = await apiFetchFichas(currentType);

    // 2) Leer lo que haya escrito en el buscador de fichas
    const input = document.getElementById('fichasSearchInput');
    const rawQuery = input ? input.value.trim() : '';

    if (rawQuery) {
      const qLower = rawQuery.toLowerCase();

      // 2a) Filtrar por nombre/teléfono (solo las que tienen alguna coincidencia)
      const filtradas = (rows || []).filter(r => {
        const nombre   = getCampoFicha(r, 'nombre','cliente').toLowerCase();
        const telefono = getCampoFicha(r, 'telefono','tel','phone').toLowerCase();
        return nombre.includes(qLower) || telefono.includes(qLower);
      });

      // 2b) Ordenar para que la mejor coincidencia quede primero
      rows = sortFichasByQuery(filtradas, rawQuery);
    }

    // 3) Render normal con la lista (ya ordenada si había búsqueda)
...
// Normaliza una hora tipo "9:5" -> "09:05" para que ordene bien
function normalizeHora(h) {
  if (!h) return '99:99'; // las que no tienen hora quedan al final
  const s = String(h).trim();
  const match = s.match(/^(\d{1,2}):(\d{1,2})$/);
  if (!match) return s; // si viniera algo raro, lo dejamos tal cual
  const hh = match[1].padStart(2, '0');
  const mm = match[2].padStart(2, '0');
  return `${hh}:${mm}`;
}

// Comparator genérico ascendente por campo .hora
function sortByHoraAsc(a, b) {
...
function renderTurnosHoy(listEl, emptyEl, items) {
  try {
    ...
    // ⬇️⬇️ NUEVO: ordenar por hora ASC (HH:MM)
    const turnosOrdenados = [...turnos].sort(sortByHoraAsc);
    // ⬆️⬆️ importante: usamos copia para no mutar el array original
    ...
    // ⬇️ Usamos la lista ordenada
    turnosOrdenados.forEach((it) => {
...
function openCalendarDayDetail(day, y, m, items) {
  window.__CAL_DAY_DETAIL = { day, year: y, month: m };

  const reservas    = items?.reservas    || [];
  const prereservas = items?.prereservas || [];
  const cumples     = items?.cumples     || items?.cumple_hoy || [];

  // ⬇️⬇️ NUEVO: ordenar por hora
  const reservasOrdenadas    = [...reservas].sort(sortByHoraAsc);
  const prereservasOrdenadas = [...prereservas].sort(sortByHoraAsc);
  ...
  const secReservas = reservasOrdenadas.length ? `
    <div class="calgroup">
      <h4>Reservas (${reservasOrdenadas.length})</h4>
  ...
  const secPreRes = prereservasOrdenadas.length ? `
    <div class="calgroup">
      <h4>Pre-reservas (${prereservasOrdenadas.length})</h4>
      ${prereservasOrdenadas.map((p, idx) => {
  ...
  if (inline) {
    ...
    // ⬇️⬇️ USAMOS LOS ARRAYS ORDENADOS
    wireCalendarDayClicks(inline, { reservas: reservasOrdenadas, prereservas: prereservasOrdenadas, cumples });
  ...
  if (typeof showDetalleModal === 'function') {
    ...
    if (modalBody) {
      // ⬇️⬇️ TAMBIÉN AQUÍ
      wireCalendarDayClicks(modalBody, { reservas: reservasOrdenadas, prereservas: prereservasOrdenadas, cumples });
    }
  }
}
...
function renderCalendarGrid(y, m, data = { reservas: [], prereservas: [], cumples: [] }) {
  ...
  for (let day = 1; day <= totalDays; day++) {
    ...
    // ⬇️ NUEVO: ordenar por hora entre reservas y prereservas
    const itemsOrdenados = [...items].sort((a, b) => {
      // cumples no tienen hora, los mandamos al final
      if (!a.hora && !b.hora) return 0;
      if (!a.hora) return 1;
      if (!b.hora) return -1;
      return sortByHoraAsc(a, b);
    });

    // colorear la celda según su contenido
    const hasR  = itemsOrdenados.some(it => it.type === 'reserva');
    const hasPR = itemsOrdenados.some(it => it.type === 'prereserva');
    const hasC  = itemsOrdenados.some(it => it.type === 'cumple');
    ...
    itemsOrdenados.forEach(it => {
      ...
      const horaText = (typeof formatHora12 === 'function' && it.hora)
        ? formatHora12(it.hora)
        : (it.hora || '');
...
// 🔹 Helper simple de debounce para no saturar al server
...
// 🔹 Render de resultados de fichas técnicas
function renderFichasResultados(items) {
  const cont   = document.getElementById('fichasResultados');
  const empty  = document.getElementById('fichasResultadosEmpty');
  const tpl    = document.getElementById('tplFichaItem');
  if (!cont || !empty || !tpl) return;

  cont.innerHTML = '';

  if (!Array.isArray(items) || !items.length) {
    empty.hidden = false;
    empty.textContent = 'No se encontraron fichas con ese nombre o teléfono.';
    return;
  }
  ...
  items.forEach(it => {
    // Clonar el bloque de ficha técnica del listado
    const li = tpl.content.firstElementChild.cloneNode(true);

    // id / tipo de ficha (para que tus handlers actuales puedan usarlo)
    li.dataset.id   = it.row || it.Row || it._row || '';
    li.dataset.type = it._tipo || '';

    // Título = nombre del cliente
    const titleEl = li.querySelector('.list__title');
    if (titleEl) {
      titleEl.textContent = it.nombre || '';
    }

    // Segunda línea: teléfono + tipo de ficha
    let metaEl = li.querySelector('.list__meta');
    if (!metaEl) {
      metaEl = document.createElement('div');
      metaEl.className = 'list__meta';
      const main = li.querySelector('.list__main');
      if (main) main.appendChild(metaEl);
    }
    const tipoFicha = it._tipo ? ` • ${it._tipo}` : '';
    metaEl.textContent = `${it.telefono || ''}${tipoFicha}`;

    // 👉 Opcional: si NO quieres permitir Editar/Eliminar desde resultados,
    // puedes ocultar esos botones (solo se quedaría "Ver ficha"):
    // const btnEdit = li.querySelector('.js-edit');
    // if (btnEdit) btnEdit.remove();
    // const btnDelete = li.querySelector('.js-delete');
...
// ----------------------------------------------------
// 🔎 Ejecutar búsqueda de fichas técnicas (UI)
// ----------------------------------------------------
async function buscarFichasTecnicas() {
  if (!fichasList || !fichasEmpty) return;
  ...
  fichasList.innerHTML = '';
  fichasEmpty.hidden = false;
  fichasEmpty.textContent = 'Buscando fichas...';

  try {
    const filas = await fetchFichasTecnicasUnificadas(query);

    if (!filas.length) {
      fichasList.innerHTML = '';
      fichasEmpty.hidden = false;
      fichasEmpty.textContent = 'No se encontraron fichas con ese nombre o teléfono.';
      return;
    }

    renderFichasList(fichasList, fichasEmpty, filas, '');

  } catch (err) {
    console.error('Error en buscarFichasTecnicas:', err);
    fichasList.innerHTML = '';
    fichasEmpty.hidden = false;
    fichasEmpty.textContent = 'Ocurrió un error al buscar fichas.';
  }
}
...
// Llamar a esto cuando se active la vista de Fichas Técnicas
document.addEventListener('DOMContentLoaded', () => {
  initFichasBuscador();
  // Si usas vistas SPA y fichasView se monta después,
  // puedes llamar a initFichasBuscador() justo cuando muestras esa vista.
});
...
// ----------------------------------------------------
// 🔍 Buscar fichas en FT.Pestañas + FT.Facial
// ----------------------------------------------------
async function fetchFichasTecnicasUnificadas(query) {
  if (!query || !query.trim()) return [];

  const q = query.toLowerCase().trim();

  try {
    const [rowsP, rowsF] = await Promise.all([
      fetchFromSheet('Pestañas'),
      fetchFromSheet('Faciales')
    ]);

    let combined = [...rowsP, ...rowsF];
    ...
    combined = combined.map(r => {
      if (!r.tipo && r._tipo) {
        return { ...r, tipo: r._tipo };
      }
      return r;
    });

    return sortFichasByQuery(combined, query);

  } catch (err) {
    console.error('Error buscando fichas unificadas:', err);
    return [];
  }
}
